...
export default function WideCard(props) {
  ...
  return (
      <Link to={`/info/${props.item.type}/${props.item.item.id}`} className="widecard flex relative w-full h-full overflow-hidden rounded-lg group">
        <img className="widecard-image group-hover:scale-105 w-[16vw] h-full object-cover transition-all duration-150" src={props.item.item.backdrop_path && `https://image.tmdb.org/t/p/w500/${props.item.item.backdrop_path}`} alt="Backdrop" />
        <div className="widecard-content group flex justify-start items-end absolute w-full h-full bottom-0 left-0 bg-gradient-to-t from-black">
          <button className='flex justify-center items-center absolute top-1 right-1 opacity-0 w-9 h-9 bg-white bg-opacity-90 border-0 p-0 rounded-lg cursor-pointer transition-all duration-150 group-hover:opacity-100' id="watchlist-button" onClick={(event) => removeItem(event, props.item.id)}>
