...
export default function WatchlistCard({ item, type, onRemove }) {
  ...
  return (
    <div className="watch-card flex relative w-[14.24vw] max-2xl:w-[17.23vw] max-xl:w-[28.49vw] max-lg:w-[43.36vw] max-md:w-[45.87vw] h-full rounded-lg text-base transition-all duration-200 overflow-hidden">
      <Link to={`/info/${type}/${item.id}`} className="watch-card-link group">
        ...
        <div className="watchlist-content absolute top-0 left-0 w-full h-full flex justify-end flex-col items-center text-center font-semibold transition-all duration-200 bg-gradient-to-t from-black rounded-md shadow-inner shadow-black">
          ...
          <button className='flex justify-center items-center absolute top-1 right-1 opacity-0 max-xl:opacity-100 w-9 h-9 z-[1111] bg-white bg-opacity-90 border-0 p-0 rounded-lg cursor-pointer transition-all duration-150 group-hover:opacity-100 hover:bg-red-500' id="watchlist-button" onClick={(event) => removeItem(event, item.id)}>
