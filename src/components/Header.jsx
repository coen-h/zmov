...
export default function Header() {
  ...
  return (
    <div className={`flex fixed h-20 max-md:h-16 w-screen z-[1111] transition-all duration-300 bg-gradient-to-b from-gray-950 ${isVisible ? 'max-xl:top-0' : 'max-xl:top-[-100px]'} ${isTop ? 'backdrop-blur-0' : 'backdrop-blur-sm'}`}>
      <div className='flex justify-between items-center flex-grow my-4 mx-12 gap-1 max-md:my-0 max-md:mx-[11px] max-xs:justify-center'>
        <Link to="/">
          <img src="/images/logo.svg" className='w-[100px] transition-all duration-150 east-in-out mt-1 hover:opacity-60 scale-[0.97] max-xs:hidden' alt="Logo" />
          <img src="/images/icon.svg" className='w-[46px] transition-all duration-150 east-in-out mt-1 hover:opacity-60 scale-[0.97] xs:hidden block' alt="Logo" />
        </Link>
